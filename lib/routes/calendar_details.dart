    ...
    return Scaffold(
      ...
      body: Calendar(
        ...
        loadOccupancies: ({start, end, occupanciesPerDay}) async {
          ...
          } else if (widget.args.mode == CalendarDetailsMode.CLASSROOM) {
            final apiInstance = ClassroomApi();

            return await apiInstance.classroomsIdOccupanciesGet(
              widget.args.id,
              start: start,
              end: end,
              occupanciesPerDay: occupanciesPerDay,
            );
          } else if (widget.args.mode == CalendarDetailsMode.CLASS) {
            final apiInstance = ClassesApi();

            return await apiInstance.classesIdOccupanciesGet(
              widget.args.id,
              start: start,
              end: end,
              occupanciesPerDay: occupanciesPerDay,
            );
          } else if (widget.args.mode == CalendarDetailsMode.SUBJECT) {
            final apiInstance = SubjectsApi();

            return await apiInstance.subjectsIdOccupanciesGet(
              widget.args.id,
              start: start,
              end: end,
              occupanciesPerDay: occupanciesPerDay,
